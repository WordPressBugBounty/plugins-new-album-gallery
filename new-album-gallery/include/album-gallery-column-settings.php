...
<style>
	...
	.gallery-settings,
	.hover_stack_effect_settings,
	.hover_overlay_effect_settings {
...
<?php
// Default settings
register_activation_hook(__FILE__, 'nag_column_defaultsettings');
function nag_column_defaultsettings()
{
...
if (isset($_POST['agp_column_setting_action'])) {
	if (isset($_POST['security'])) {
		$ag_save_nonce_value = sanitize_text_field(wp_unslash($_POST['security']));
	} else {
		$ag_save_nonce_value = '';
	}
...
<script>
	function AgSaveSettings() {
		...
		jQuery.ajax({
			...
			url: location.href,
			cache: false,
			data: jQuery('#agp-column-settings').serialize() + '&agp_column_setting_action=save_agsetting' + '&security=' + '<?php echo esc_attr(wp_create_nonce('ag_save_nonce')); ?>',
			complete: function() {},
